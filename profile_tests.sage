    ...
    try:
        with open(filename, 'w') as file:
            with contextlib.redirect_stdout(file):
              ...
              alg_profile_old = [ log( G.Rr[i][i].alg_norm(),2 ).n(50) for i in range(n) ]
              ...
              G = GSO_obj(B_red)
              G.compute_GSO(start=0, end=n)
              alg_profile = [ log( G.Rr[i][i].alg_norm(),2 ).n(50) for i in range(n) ]
              ...
              observed_value = sum( (n-i-1)*dr[i] for i in range(n-1) ) / n
...
def process_output( output ):
    ...
    print( "perfect_value | observed_value | log_alpha | log_slope" )
    for key in d.keys():
        o = d[key]
...
f=64
k=4
qs = [ next_prime( ceil(2^tmp) ) for tmp in [20,25,30] ] * tests_per_q
beta=35
