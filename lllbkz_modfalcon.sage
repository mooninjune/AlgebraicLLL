    ...
    with open(filename, 'w') as file:
        with contextlib.redirect_stdout(file):
            ...
            if d*(k+1)<=192 and q<2^22:
                ...
                G = GSO.Mat( B_red,float_type='mpfr' )
            G.update_gso()
            print(f"Initial r00^2: {G.get_r(0,0)}")
            sys.stdout.flush()
            ...
            for beta_counter in range(4,beta+1,2):
                ...
                M = bkz_obj.M
                f1, g1 =  K( list(M.B[0])[:d] ), K( list(M.B[0])[d:] )
                len_bkz = enorm_vector_over_numfield( vector(f1,g1) )^0.5
                if len_bkz < gsnorm:
...
qs = [ next_prime( ceil(2^tmp) ) for tmp in [17.0] ] * tests_per_q
beta=14
...
for k in d.keys():
    print( f"{k} {np.mean(d[k][0])} {np.mean(d[k][1])} {np.mean(d[k][2])} {np.mean(d[k][3])} {np.mean(d[k][4])} {d[k][5]} {np.mean(d[k][6])+np.mean(d[k][7])}" )
